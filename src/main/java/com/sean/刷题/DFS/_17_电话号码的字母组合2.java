...
public class _17_电话号码的字母组合2 {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        dsf(0, chars, string, list);
        return list;
    }

    private void dsf(int idx, char[] chars, char[] string, List<String> list) {
        // 已经进入到最后一层了，不能再往下了
        if (idx == chars.length) {
            // 得到一个正确的结果
        ...
        for (char letter : letters) {
            string[idx] = letter;
            dsf(idx + 1, chars, string, list);
        }
    }

    public static void main(String[] args) {
        _17_电话号码的字母组合2 test = new _17_电话号码的字母组合2();
