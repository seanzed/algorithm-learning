 ...
 * 给出一个满足下述规则的二叉树：
 *
 * root.val == 0 如果 treeNode.val == x 且 treeNode.left != null，那么 treeNode.left.val == 2 * x + 1 如果
 * treeNode.val == x 且 treeNode.right != null，那么 treeNode.right.val == 2 * x + 2
 ...
 * findElements.find(2); // return True findElements.find(3); // return False findElements.find(4);
 * // return False findElements.find(5); // return True  
 *
 * 提示：
 *
 * TreeNode.val == -1 二叉树的高度不超过 20 节点的总数在 [1, 10^4] 之间 调用 find() 的总次数在 [1, 10^4] 之间 0 <= target <=
 * 10^6
 *
 * 链接：https://leetcode-cn.com/problems/find-elements-in-a-contaminated-binary-tree
 *
 * @author chenxu
 * @summary _1261_在受污染的二叉树中查找元素
 * @since 2020-08-31 23:41
 */
